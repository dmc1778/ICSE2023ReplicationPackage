...
def run(library_name, opt, filename):
    ...
    if library_name == 'tensorflow':
        ...
        os.chdir(os.path.join(this_project))
...
def diff_based_matching(changed_lines, current_commit, detector_name, library_name, opt):
    ...
    if os.path.isfile(os.path.join(this_project,current_commit.filename)):
        [output, execution_time] = run(library_name, opt, current_commit.filename)
...
def fixed_warning_base_matching(fix_commit, vul_commit, detector_name, library_name, opt):
    ...
    if os.path.isfile(os.path.join(this_project, vul_commit.filename)):
        ...
        [output1, execution_time1] = run(library_name, opt, vul_commit.filename)
    ...
    if os.path.isfile(os.path.join(this_project, vul_commit.filename)):
        [output2, execution_time2] = run(library_name, opt, vul_commit.filename)
                ...
                for counter, item in enumerate(data):
                    ...
                    for mod in current_commit.modifications:
                        if 'test' not in mod.new_path and 'test' not in mod.filename and mod.filename.split('.')[-1] in _extensions:
                            ...
                            if mapping_ == 'diff':
                                detection_status, vul_file_object, res, execution_time = diff_based_matching(cl, mod, 'infer', user_names[i], opt[0])
                                if res == 'not detected':
                                    print('No vulnerable candidate detected!')
                                    my_data = ['infer', 'diff', dir.split('_')[1].split('.')[0], execution_time, commit_base_link+x[0], commit_base_link+current_commit.hash, vul_file_object.filename, vul_file_object.new_path, vul_file_object.added, vul_file_object.removed, 0]
                                    my_data.append('not detected')

                                else:
                                    data_list, j = combine_diff_results(detection_status)
                                    my_data = ['infer', 'diff', dir.split('_')[1].split('.')[0], execution_time, commit_base_link+x[0], commit_base_link+current_commit.hash, vul_file_object.filename, vul_file_object.new_path, vul_file_object.added, vul_file_object.removed, j]
                            ...
                            if mapping_ == 'fixed':
                                flag, vul_file_object, res1, res2, execution_time = fixed_warning_base_matching(cl, mod, 'infer', user_names[i], opt[0])
                                    
                                if flag:
                                    data_list, j = combine_fixed_results(res1[0])
                                    my_data = ['infer', 'fixed' , dir.split('_')[1].split('.')[0], execution_time, commit_base_link+x[0], commit_base_link+current_commit.hash, vul_file_object.filename, vul_file_object.new_path, vul_file_object.added, vul_file_object.removed, j]
                                    my_data = my_data + data_list
                
                                else:
                                    my_data = ['infer', 'fixed' , dir.split('_')[1].split('.')[0], execution_time, commit_base_link+x[0], commit_base_link+current_commit.hash, vul_file_object.filename, vul_file_object.new_path, vul_file_object.added, vul_file_object.removed, 0 , 'not detected']
